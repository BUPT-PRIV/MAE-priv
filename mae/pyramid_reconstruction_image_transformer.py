...
class PriTDncoder(nn.Module):
    ...
    def __init__(self, encoder: PriTEncoder, stage_idx, decoder_dim=512, decoder_depth=8):
        ...
        stride = encoder.patch_size * reduce(mul, encoder.strides[:stage_idx])
        out_size = (img_size[0] // stride, img_size[1] // stride)
        ...
        self.num_visible = encoder.num_visible
        self.stride = stride
        ...
        self.decoder_norm = encoder.norm_layer(decoder_dim, eps=1e-6)
        decoder_out_dim = (patch_size if encoder.pyramid_reconstruction else stride) ** 2 * 3
        self.decoder_linear_proj = nn.Linear(decoder_dim, decoder_out_dim)
        # pos_embed
    ...
    def forward(self, x, shuffle):
        ...
        B, N, L = encoded_visible_patches.shape
        ...
        mask_token = self.mask_token.repeat([B, num_masked, G, 1])  # B x M x G x D
        ...
        # decode all tokens
        decoded_all_token = self.decoder_blocks(all_tokens)  # B x (P*G) x D
        decoded_all_token = decoded_all_token.view(B, self.num_patches, -1, L)  # B x P x G x D
        decoded_masked_token = decoded_all_token[:, masked_inds] if num_masked > 0 else decoded_all_token  # B x M x G x D
        decoded_masked_token = decoded_masked_token.reshape(B, -1, L)  # B x (M*G) x D
        decoded_masked_token = self.decoder_norm(decoded_masked_token)  # B x (M*G) x D
        decoded_masked_token = self.decoder_linear_proj(decoded_masked_token)  # B x (M*G) x O*3

        return decoded_masked_token
